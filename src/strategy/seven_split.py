...
def calc_strategy_profile(prices: list[float], highs: list[float] | None = None,
    ...
    if not custom_inst:
        score, reasons = score_default_strategy(
            prices, highs, volumes,
            current=current, previous=prev, rsi14=rsi14, rsi2=rsi2,
            sma20=sma20, sma60=sma60, sma120=sma120, bb_lo=bb_lo,
            macd=macd, ma_cross=ma_cross, momentum=momentum, atr_pct=atr_pct,
            value_surge=value_surge, wave_pullback=wave_pullback,
            config=config, calc_rsi=calc_rsi, calc_bollinger=calc_bollinger,
        )
