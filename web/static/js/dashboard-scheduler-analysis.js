(function (global) {
    ...
    function checkCell(ok, value) {
        return `<td class="scheduler-analysis-check ${ok ? 'is-pass' : 'is-fail'}">${value || (ok ? '통과' : '제외')}</td>`;
    }
    ...
    function render(roundData, summary, details, deps) {
        ...
        if (details && roundData.analysisRows.length) {
            ...
            const cells = (row) => alpha
                ? `${checkCell(row.checks?.alpha_reversal)}${checkCell(row.checks?.price_confirmed, row.checks?.price_confirmed ? '가점' : '미가점')}<td>${row.adx == null ? '-' : deps.formatNumber(row.adx, 1)} · ${row.checks?.trend_quality_ok ? '통과' : '제외'}</td><td>${row.atr_pct == null ? '-' : `${deps.formatNumber(row.atr_pct, 2)}%`} · ${row.checks?.volatility_ok ? '통과' : '제외'}</td>`
                : `${checkCell(row.checks?.oversold_seen)}${checkCell(row.checks?.price_confirmed)}<td>${row.stop_distance_pct == null ? '-' : `${deps.formatNumber(row.stop_distance_pct, 2)}%`} · ${row.checks?.risk_acceptable ? '통과' : '제외'}</td><td>${row.rsi == null ? '-' : deps.formatNumber(row.rsi, 1)}</td>`;
            details.innerHTML = `<details class="scheduler-analysis-details"><summary><strong>종목별 조건 점검 ${roundData.analysisRows.length}건</strong></summary><div class="table-responsive scheduler-analysis-table-wrap"><table class="scheduler-analysis-table"><thead><tr><th>종목</th><th>점수</th><th>EMA200 추세</th>${headers}<th>결과/사유</th></tr></thead><tbody>${roundData.analysisRows.map((row) => `<tr><td><strong>${deps.escapeHtml(row.name || row.symbol || '-')}</strong><br><small>${deps.escapeHtml(row.symbol || '')}</small></td><td>${deps.formatNumber(row.score || 0, 2)}</td>${checkCell(row.checks?.trend_ok)}${cells(row)}<td>${row.checks?.entry_ready ? deps.pill('진입 가능', 'buy') : deps.pill('조건 미충족', 'hold')}<br><small>${deps.escapeHtml((row.reasons || []).join(' · '))}</small></td></tr>`).join('')}</tbody></table></div></details>`;
        }
    }
